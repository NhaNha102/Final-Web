...
function displayFeaturedProducts() {
    ...
    featuredProducts.forEach(product => {
        ...
        const addButton = productDiv.querySelector('.action-btn');
        addButton.addEventListener('click', () => {
            addToCart(product); // Gọi hàm addToCart từ script.js
        });
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    if (window.location.pathname.includes('MainPage.html')) {
        // Kiểm tra xem addToCart có được định nghĩa không
        if (typeof addToCart === 'undefined') {
            console.error('addToCart is not defined. Ensure script.js is loaded before MainPage.js');
        }
