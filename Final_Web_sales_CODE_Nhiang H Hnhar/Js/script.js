...
function addToCart(product) {
    if (!checkLogin()) {
        // Lưu URL trang hiện tại vào localStorage
        localStorage.setItem('returnUrl', window.location.href);
        window.location.href = '../HTML/Login.html';
        return;
    }
    if (!product || !product.name) {
        showCustomNotification("Error: Product information is missing!");
        return;
    }
    ...
    updateCartCount();
    showCustomNotification(`${product.name} has been added to your cart!`);
}
// Hàm hiển thị thông báo tùy chỉnh
function showCustomNotification(message, type = 'success') {
    // Tạo div thông báo
    const notification = document.createElement('div');
    notification.className = `custom-notification ${type}`; // Thêm class để tùy chỉnh kiểu thông báo
    ...
    notification.style.transform = 'translate(-50%, -50%)'; // Căn giữa màn hình
    notification.style.padding = '15px 30px';
    notification.style.backgroundColor = type === 'success' ? '#4CAF50' : '#f44336'; // Xanh cho success, đỏ cho error
